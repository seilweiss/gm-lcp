    ...
    switch(*(instruction32++))
    {
      ...
      case BC_OP_NOT :
      {
        ...
        { 
          ...
          if(res == SYS_EXCEPTION) goto exception;
      ...
      case BC_GETIND :
      {
        operand = top - 2;
        --top;
        gmOperatorFunction op = OPERATOR(operand->m_type, O_GETIND);
        if(op)
        {
          op(this, operand);
        }
...
{
  if((m_top + a_extra + GMTHREAD_SLACKSPACE) >= m_size)
  {
