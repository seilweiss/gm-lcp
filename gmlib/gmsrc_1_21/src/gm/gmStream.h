...
public:
    ...
    virtual unsigned int Read(void* p_buffer, unsigned int p_n, bool p_swap = false) = 0;

    /// \brief Write() will write p_n bytes from p_buffer to the stream.
    /// \return the number of bytes successfully written
    virtual unsigned int Write(const void* p_buffer, unsigned int p_n, bool p_swap = false) = 0;
