...
unsigned int gmStreamBufferStatic::Seek(unsigned int p_pos)
{
    ...
    int cursor = p_pos;
    if (cursor < 0) return ILLEGAL_POS;
    if ((unsigned int)cursor > m_size) return ILLEGAL_POS;
    m_cursor = cursor;
    return oldCursor;
}
...
unsigned int gmStreamBufferStatic::Read(void* p_buffer, unsigned int p_n, bool p_swap)
{
    ...
    // swap hack
    if (p_swap && GetSwapEndianOnWrite())
    {
        for (unsigned int i = 0; i < p_n / sizeof(gmuint32); i++)
        {
            SwapEndian(((gmuint32*)p_buffer)[i]);
        }
    }
...
unsigned int gmStreamBufferStatic::Write(const void* p_buffer, unsigned int p_n, bool p_swap)
{
    m_flags |= F_ERROR;
    return 0;
}
...
unsigned int gmStreamBufferDynamic::Read(void* p_buffer, unsigned int p_n, bool p_swap)
{
...
unsigned int gmStreamBufferDynamic::Write(const void* p_buffer, unsigned int p_n, bool p_swap)
{
    unsigned int remain = m_stream.Count() - m_cursor;
    if (p_n > remain)
    {
    ...
    if (p_swap && GetSwapEndianOnWrite())
    {
        for (unsigned int i = 0; i < p_n / sizeof(gmuint32); i++)
        {
            SwapEndian(((gmuint32*)(m_stream.GetData() + m_cursor))[i]);
        }
    }
