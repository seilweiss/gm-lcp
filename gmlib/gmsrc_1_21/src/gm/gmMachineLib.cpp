...
static int GM_CDECL gmDoString(gmThread * a_thread) // string, now(int), returns thread id, null on error, exception on compile error
{
  ...
  if(script)
  {
    int errors = a_thread->GetMachine()->ExecuteString(script, &id, (now) ? true : false);
    if(errors)
    {
      return GM_EXCEPTION;
    }
...
{
  GM_CHECK_NUM_PARAMS(1);
  GM_INT_PARAM(id, 0, GM_INVALID_THREAD);
  ...
  if(id != a_thread->GetId())
  {
    ...
    if(thread)
    {
      thread->GetMachine()->Sys_SwitchState(thread, gmThread::KILLED);
    }
    return GM_OK;
  }
  return GM_SYS_KILL;  
...
static bool GM_CDECL gmGCTraceStateUserType(gmMachine * a_machine, gmUserObject* a_object, gmGarbageCollector* a_gc, const int a_workLeftToDo, int& a_workDone)
{
  gmStateUserType * state = (gmStateUserType *) a_object->m_user;
...
{ 
  ...
  */
  ...
  /*gm
    ...
    \param string operator name is one of
          ...
          "getdot", "getdot", "getind", "setind", "add", "sub", "mul", "div", "mod",
  ...
  */
  ...
  /*gm
    ...
    \param string script
    \param int now optional (1) set as true and the string will execute before returning to this thread
  ...
  */
  ...
  /*gm
    \function block
    \brief block will block on all passed vars, execution will halt until another thread signals one of the block variables.
    \param ... vars
