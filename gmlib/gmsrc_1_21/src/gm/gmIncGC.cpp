...
//    typedef bool (GM_CDECL *gmGCTraceCallback)(gmMachine * a_machine, gmUserObject* a_object, gmGarbageCollector* a_gc, const int a_workLeftToDo, int& a_workDone);
//
...
void gmGCColorSet::Init(gmGarbageCollector* a_gc)
{
  ...
  //
  m_gray = &m_headObject;
  m_scan = &m_separatorObject;
  m_free = &m_separatorObject;
...
bool gmGCColorSet::VerifyIntegrity()
{
  ...
  int curCol = GC_DEBUG_COL_GRAY;
  while(curObj != m_tail)
  {
    if(curObj == m_scan)
    {
      curCol = GC_DEBUG_COL_BLACK;
    }
    if(curObj == m_free)
    {
      curCol = GC_DEBUG_COL_FREE;
    }
    if(curObj == m_white)
    {
      curCol = GC_DEBUG_COL_WHITE;
    }
...
bool gmGCColorSet::BlackenNextGray(int& a_workDone, int a_workLeftToDo)
{
...
#if GC_DEBUG
      GM_ASSERT(m_scan->m_curPosColor == GC_DEBUG_COL_GRAY);
      m_scan->m_curPosColor = GC_DEBUG_COL_BLACK;
#endif //GC_DEBUG

      a_workDone = FollowPointers(a_workLeftToDo);
...
#if GC_DEBUG
  GM_ASSERT(a_obj->m_curPosColor == GC_DEBUG_COL_WHITE);
  a_obj->m_curPosColor = GC_DEBUG_COL_GRAY;
#endif //GC_DEBUG
...
void gmGCColorSet::ReclaimGarbage()
{
  ...
  if (m_white->GetNext() != m_tail)       // There are garbage objects
  {
    ...
    for(gmGCObjBase* temp = m_white->GetNext();
        ...
        temp = temp->GetNext())
    {
      GM_ASSERT(temp->m_curPosColor == GC_DEBUG_COL_WHITE);
      temp->m_curPosColor = GC_DEBUG_COL_FREE;
    }
#endif //GC_DEBUG

    // Reclaim the garbage.
    m_white->GetNext()->SetPrev(m_free->GetPrev());
    m_tail->GetPrev()->SetNext(m_free);
    m_free->GetPrev()->SetNext(m_white->GetNext());
    m_free->SetPrev(m_tail->GetPrev());

    if (m_scan == m_free)  // There are no black objects
    {  
      m_scan = m_white->GetNext();
    }
    m_free = m_white->GetNext();
  ...
  if (m_scan != m_free)   // There are live (black) objects
  {
    ...
    for(gmGCObjBase* temp = m_scan;//m_scan->GetNext();
        ...
        temp = temp->GetNext())
    {
      GM_ASSERT(temp->m_curPosColor == GC_DEBUG_COL_BLACK);
      temp->m_curPosColor = GC_DEBUG_COL_WHITE;
    }
#endif //GC_DEBUG
...
#if GC_DEBUG
      //GM_ASSERT(objToRecycle->m_curPosColor == GC_DEBUG_COL_WHITE);
      GM_ASSERT(objToRecycle->m_curPosColor == GC_DEBUG_COL_FREE);
      objToRecycle->m_curPosColor = GC_DEBUG_COL_INVALID;
#endif //GC_DEBUG
...
#if GC_DEBUG
  GM_ASSERT(a_obj->m_curPosColor == GC_DEBUG_COL_INVALID);
  a_obj->m_curPosColor = GC_DEBUG_COL_BLACK;
#endif //GC_DEBUG
...
void gmGarbageCollector::Init(gmGCScanRootsCallBack a_scanRootsCallback, gmMachine* a_gmMachine)
{
  ...
  m_workPerIncrement = GC_DEFAULT_WORK_INCREMENT;
  m_maxObjsToDestructPerIncrement = GC_DEFAULT_DESTRUCT_INCREMENT;
  m_workLeftToDo = 0;
...
bool gmGarbageCollector::BlackenGrays() 
{
  ...
  while(m_colorSet.AnyGrays()) 
  {
    ...
    workDone = m_workLeftToDo;

    while(m_colorSet.BlackenNextGray(workDone, m_workLeftToDo))
    {
      m_workLeftToDo -= workDone;
      if (m_workLeftToDo <= 0)
      {
        // Quit early
...
bool gmGarbageCollector::Collect()
{
  if(m_fullThrottle)
  {
    m_workLeftToDo = GM_MAX_INT32;
  }
  else
  {
    m_workLeftToDo = m_workPerIncrement;
  }
  ...
  m_gcTurnedOff = true;
#else //GC_TURN_OFF_ABLE  
  Flip();
#endif //GC_TURN_OFF_ABLE  
