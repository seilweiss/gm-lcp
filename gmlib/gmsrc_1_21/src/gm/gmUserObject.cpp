...
bool gmUserObject::Trace(gmMachine * a_machine, gmGarbageCollector* a_gc, const int a_workLeftToDo, int& a_workDone)
{
  gmGCTraceCallback gcTrace = a_machine->GetUserGCTraceCallback((gmType) m_userType);
  if(gcTrace)
  {
    return gcTrace(a_machine, this, a_gc, a_workLeftToDo, a_workDone);
  }
  else
  {
    ++a_workDone;
