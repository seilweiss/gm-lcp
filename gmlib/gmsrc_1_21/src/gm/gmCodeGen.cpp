...
public:
  ...
  LoopInfo * m_currentLoop; //!< loop top of stack
  ...
  LoopInfo * PushLoop();
  LoopInfo * PopLoop();
  void ApplyPatches(Patch * a_patches, gmByteCodeGen * a_byteCode, gmuint32 a_value);
};
...
bool gmCodeGenPrivate::GenStmtBreak(const gmCodeTreeNode * a_node, gmByteCodeGen * a_byteCode)
{
  ...
  if(m_currentLoop)
  {
    ...
    patch->m_next = m_currentLoop->m_breaks;
    m_currentLoop->m_breaks = patch;
    return true;
  }
...
bool gmCodeGenPrivate::GenStmtContinue(const gmCodeTreeNode * a_node, gmByteCodeGen * a_byteCode)
{
  ...
  if(m_currentLoop)
  {
    ...
    Patch * patch = &m_patches.InsertLast();
    patch->m_address = a_byteCode->Skip(sizeof(gmuint32));
    patch->m_next = m_currentLoop->m_continues;
    m_currentLoop->m_continues = patch;
    return true;
  }
...
bool gmCodeGenPrivate::GenStmtFor(const gmCodeTreeNode * a_node, gmByteCodeGen * a_byteCode)
{
  ...
  ApplyPatches(m_currentLoop->m_breaks, a_byteCode, loc1);
  ApplyPatches(m_currentLoop->m_continues, a_byteCode, continueAddress);
...
bool gmCodeGenPrivate::GenStmtForEach(const gmCodeTreeNode * a_node, gmByteCodeGen * a_byteCode)
{
  ...
  ApplyPatches(m_currentLoop->m_breaks, a_byteCode, breakAddress);
  ApplyPatches(m_currentLoop->m_continues, a_byteCode, continueAddress);
...
gmCodeGenPrivate::LoopInfo * gmCodeGenPrivate::PushLoop()
{
  m_currentLoop = &m_loopStack.InsertLast();
  m_currentLoop->m_breaks = NULL;
  m_currentLoop->m_continues = NULL;
  return m_currentLoop;
}



gmCodeGenPrivate::LoopInfo * gmCodeGenPrivate::PopLoop()
{
  m_loopStack.RemoveLast();
  if(m_loopStack.Count())
  {
    m_currentLoop = &m_loopStack[m_loopStack.Count() - 1];
  }
  else
  {
    m_currentLoop = NULL;
...
void gmCodeGenPrivate::ApplyPatches(Patch * a_patches, gmByteCodeGen * a_byteCode, gmuint32 a_value)
{
  unsigned int pos = a_byteCode->Tell();
  while(a_patches)
  {
    a_byteCode->Seek(a_patches->m_address);
    *a_byteCode << a_value;
    a_patches = a_patches->m_next;
  }
  a_byteCode->Seek(pos);
}
