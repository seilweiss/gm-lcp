...
#define GC_TURN_OFF_ABLE 1                    // Let GC turn off after completion, can be turned back on when memory low
#define GC_KEEP_PERSISTANT_SEPARATE 1         // Keep persistant object in separate list for efficiency
#define DEPTH_FIRST 1                         // Depth first or bredth first tracing
#ifdef _DEBUG
  #define GC_STATS 1                          // Some stats
#else
  #define GC_STATS 0
#endif

#define GC_DEFAULT_WORK_INCREMENT 100         // Desired number of objects to trace per frame
#define GC_DEFAULT_DESTRUCT_INCREMENT 100     // Desired number of old objects to free per frame

#define GC_DEBUG 0                            // GC Debugging paranoid check code.  Only set to 1 when debugging the GC routines.
...
enum
{
  GC_DEBUG_COL_INVALID, //0
  GC_DEBUG_COL_GRAY,    //1
  GC_DEBUG_COL_BLACK,   //2
  GC_DEBUG_COL_WHITE,   //3
  GC_DEBUG_COL_FREE,    //4
};
#endif //GC_DEBUG
...
public:
  ...
  gmGCObjBase()
  {
    m_curPosColor = GC_DEBUG_COL_INVALID;
  }
  int m_curPosColor;
#endif //GC_DEBUG
  ...
  /// until it has traced all pointers, or used up the a_workLeftToDo count.
  /// \param a_workLeftToDo Number of pointers to trace.
  /// \param a_workDone, The number of pointers traced.
  /// \return true if finished tracing this object, false if not finished.
  virtual bool Trace(gmMachine * a_machine, gmGarbageCollector* a_gc, const int a_workLeftToDo, int& a_workDone) 
  {
...
public:
  ...
  bool BlackenNextGray(int& a_workDone, int a_workLeftToDo);
  ...
  void MakePersistant(gmGCObjBase* a_obj)
  {
    if(m_scan == a_obj)
    {
      m_scan = a_obj->GetPrev();
    }
    if(m_free == a_obj)
    {
      m_free = a_obj->GetNext();
    }
...
protected:
  ...
  int m_workLeftToDo;                             ///< How much work left in this increment
