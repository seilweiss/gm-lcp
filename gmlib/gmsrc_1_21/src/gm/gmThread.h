...
public:
  ...
  inline gmVariable * GetTop() { return &m_stack[m_top]; }

  /// \brief GetBottom() will return the bottom of stact
  inline gmVariable * GetBottom() { return m_stack; }
  ...
  inline gmVariable * GetBase() { return &m_stack[m_base]; }
  ...
  inline gmptr ParamInt(int a_param, gmptr a_default = 0);
  inline gmfloat ParamFloat(int a_param, gmfloat a_default = 0.0f);
  inline const char * ParamString(int a_param, const char * a_default = "");
  inline gmStringObject * ParamStringObject(int a_param);
  inline gmTableObject * ParamTable(int a_param);
  inline gmFunctionObject * ParamFunction(int a_param);
  inline void * ParamUser(int a_param, int * a_userType = NULL);
  inline void * ParamUserCheckType(int a_param, int a_userType);
  inline void * ParamUser_NoCheckTypeOrParam(int a_param); //For macro
  inline gmUserObject * ParamUserObject(int a_param);
  inline gmVariable Param(int a_param);
  inline gmType ParamType(int a_param);
  inline gmptr ParamRef(int a_param);
...
inline gmptr gmThread::ParamInt(int a_param, gmptr a_default)
{
...
inline gmfloat gmThread::ParamFloat(int a_param, gmfloat a_default)
{
  if(a_param >= m_numParameters) return a_default;
  gmVariable * var = m_stack + m_base + a_param;
...
const char * gmThread::ParamString(int a_param, const char * a_default)
{
  if(a_param >= m_numParameters) return a_default;
  gmVariable * var = m_stack + m_base + a_param;
...
gmStringObject * gmThread::ParamStringObject(int a_param)
{
...
gmTableObject * gmThread::ParamTable(int a_param)
{
  if(a_param >= m_numParameters) return NULL;
  gmVariable * var = m_stack + m_base + a_param;
...
gmFunctionObject * gmThread::ParamFunction(int a_param)
{
  if(a_param >= m_numParameters) return NULL;
  gmVariable * var = m_stack + m_base + a_param;
...
void * gmThread::ParamUser(int a_param, int * a_userType)
{
  if(a_param >= m_numParameters) return NULL;
  gmVariable * var = m_stack + m_base + a_param;
...
void * gmThread::ParamUserCheckType(int a_param, int a_userType)
{
  if(a_param >= m_numParameters) return NULL;
  gmVariable * var = m_stack + m_base + a_param;
...
void * gmThread::ParamUser_NoCheckTypeOrParam(int a_param)
{
...
gmUserObject * gmThread::ParamUserObject(int a_param)
{
  if(a_param >= m_numParameters) return NULL;
  gmVariable * var = m_stack + m_base + a_param;
  if(var->m_type >= GM_USER)
...
inline gmType gmThread::ParamType(int a_param)
{
  return (m_stack + m_base + a_param)->m_type;
}


inline gmptr gmThread::ParamRef(int a_param)
{
...
#define EXCEPTION_MSG GM_THREAD_ARG->GetMachine()->GetLog().LogEntry

#define GM_CHECK_NUM_PARAMS(A) \
  if(GM_THREAD_ARG->GetNumParams() < (A)) { EXCEPTION_MSG("expecting %d param(s)", (A)); return GM_EXCEPTION; }

#define GM_CHECK_INT_PARAM(VAR, PARAM) \
  if(GM_THREAD_ARG->ParamType((PARAM)) != GM_INT) { EXCEPTION_MSG("expecting param %d as int", (PARAM)); return GM_EXCEPTION; } \
  int VAR = GM_THREAD_ARG->ParamRef((PARAM));

#define GM_CHECK_FLOAT_PARAM(VAR, PARAM) \
  if(GM_THREAD_ARG->ParamType((PARAM)) != GM_FLOAT) \
  { EXCEPTION_MSG("expecting param %d as float", (PARAM)); return GM_EXCEPTION; } \
  float VAR = GM_THREAD_ARG->Param((PARAM)).m_value.m_float;

#define GM_CHECK_STRING_PARAM(VAR, PARAM) \
  if(GM_THREAD_ARG->ParamType((PARAM)) != GM_STRING) { EXCEPTION_MSG("expecting param %d as string", (PARAM)); return GM_EXCEPTION; } \
  const char * VAR = (const char *) *((gmStringObject *) GM_OBJECT(GM_THREAD_ARG->ParamRef((PARAM))));

#define GM_CHECK_FUNCTION_PARAM(VAR, PARAM) \
  if(GM_THREAD_ARG->ParamType((PARAM)) != GM_FUNCTION) { EXCEPTION_MSG("expecting param %d as function", (PARAM)); return GM_EXCEPTION; } \
  gmFunctionObject * VAR = (gmFunctionObject *) GM_OBJECT(GM_THREAD_ARG->ParamRef((PARAM)));

#define GM_CHECK_TABLE_PARAM(VAR, PARAM) \
  if(GM_THREAD_ARG->ParamType((PARAM)) != GM_TABLE) { EXCEPTION_MSG("expecting param %d as table", (PARAM)); return GM_EXCEPTION; } \
  gmTableObject * VAR = (gmTableObject *) GM_OBJECT(GM_THREAD_ARG->ParamRef((PARAM)));

#define GM_CHECK_USER_PARAM(OBJECT, TYPE, VAR, PARAM) \
  if(GM_THREAD_ARG->ParamType((PARAM)) != (TYPE)) { EXCEPTION_MSG("expecting param %d as user type %d", (PARAM), (TYPE)); return GM_EXCEPTION; } \
