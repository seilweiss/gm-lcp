...
static int GM_CDECL gmfDoFile(gmThread * a_thread) // filename, now (1), return thread id, null on error, exception on compile error.
{
  ...
  if(filename)
  {
    ...
    int errors = a_thread->GetMachine()->ExecuteString(string, &id, (now) ? true : false, filename);
    delete[] string;
    if(errors)
    {
      return GM_EXCEPTION;
...
static int GM_CDECL gmfFileSeek(gmThread * a_thread) //
{
  gmUserObject * fileObject = a_thread->ThisUserObject();
  GM_ASSERT(fileObject->m_userType == s_gmFileType);
...
{ 
  ...
  */
  ...
  /*gm
    ...
    \param int now (1) as 1 will execute string before returning, 0 will execute later.
...
{ 
  ...
  */
  ...
  /*gm
    \function Seek
    \brief Seek NOT YET IMPLEMENTED
  */
  {"Seek", gmfFileSeek},
  /*gm
    \function Tell
